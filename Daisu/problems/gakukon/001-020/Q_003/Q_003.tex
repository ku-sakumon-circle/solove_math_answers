\begin{prob}{003}{\hosi 1}{Daisu gakukon テンプレート問題}
座標空間上の点$\mr{P}_{0} = (1,-3,1) = (x_0,y_0,z_0)$がある。コイントスを連続して$n$回行い, その結果に応じて次のように点$\mr{P}_{1},\mr{P}_{2},\cdots, \mr{P}_{n}$を順番に定める:
\[\mr{P}_{k} = (x_{k} , y_{k} , z_{k}) = \bcas (-x_{k-1}, -y_{k-1}-z_{k-1}, x_{k-1}+y_{k-1}+z_{k-1}) \quad (k回目で表が出たとき)\\ (z_{k-1}, y_{k-1}, x_{k-1}) \quad (k回目で裏が出たとき) \ecas\]
$\mr{P}_{n} = (0,0,0)$である確率を$n$を用いて表せ。
\end{prob}
...
\begin{comment}
よって$d_{n+1} = \dfrac{c_{n}}{2}$より, $n\geq 1$において
\beqn
d_n = \dfrac{1}{2}\cdot \parena{1+\dfrac{2}{\sqrt{5}}}\parena{\dfrac{\phi}{2}}^{n-1} + \dfrac{1}{2}\cdot\parena{1-\dfrac{2}{\sqrt{5}}}\parena{\dfrac{\psi}{2}}^{n-1}  - \dfrac{n+1}{2^{n}}
\eeqn
この式は$n=0$でも正しいことが分かる。\par
最後に, $e_{n+1} - e_n = \dfrac{d_{n}}{2}$ ($n\geq 0$), $e_1 = 0$であるから
\[e_{n} = \disp\sum_{k=1}^{n-1} (e_{k+1} - e_k) + e_1  = \dfrac{1}{2}\disp\sum_{k=1}^{n-1}d_{k}\]
である。$A_3:=\dfrac{5+2\sqrt{5}}{10} $, $A_4:= \dfrac{5-2\sqrt{5}}{10}$とおき, $\phi^{-2} = \psi^{2}$, $2-\phi = \psi^2$, $2-\psi = \phi^2$などに注意すると
\beqn
&&\dfrac{1}{2} \disp\sum_{k=1}^{n-1} d_k\\
&=& \dfrac{A_3}{2} \disp\sum_{k=1}^{n-1} \parena{\dfrac{\phi}{2}}^{k-1} + \dfrac{A_4}{2}\disp\sum_{k=1}^{n-1} \parena{\dfrac{\psi}{2}}^{k-1} + \disp\sum_{k=1}^{n-1} \dfrac{k+1}{2^{k+1}}\\
&=& \dfrac{A_3}{2} \dfrac{1- (\phi/2)^{n-1}}{1-(\phi/2)} + \dfrac{A_4}{2} \dfrac{1-(\psi/2)^{n-1}}{1-(\psi/2)} + \disp\sum_{k=2}^{n} \dfrac{k}{2^{k}}\\
&=& \dfrac{A_3}{\psi^{2}} \parena{1- (\phi/2)^{n-1}} + \dfrac{A_4}{\phi^{2}} \parena{1-(\psi/2)^{n-1}}\\
&&\qquad\qquad + \disp\sum_{k=2}^{n} \parena{\dfrac{1}{2^{k-1}} + \dfrac{k-1}{2^{k-1}} - \dfrac{k}{2^{k}}}\\
&=& A_3\phi^{2}\parena{1-(\phi/2)^{n-1}} + A_4\psi^{2} \parena{1-(\psi/2)^{n-1}} \\
&& \qquad\qquad+ \parena{1-\dfrac{1}{2^{n-1}} + \dfrac{1}{2} - \dfrac{n}{2^{n}} } \\
&=& -4A_3(\phi/2)^{n+1} - 4A_4(\psi/2)^{n+1}  - \dfrac{n+2}{2^{n}} + (A_3\phi^2 + A_4\psi^2 + \dfrac{3}{2})\\
&=& -4A_3(\phi/2)^{n+1} - 4A_4(\psi/2)^{n+1}  - \dfrac{n+2}{2^{n}} \\
&&\qquad \qquad + (A_3\dfrac{3+\sqrt{5}}{2} + A_4\dfrac{3-\sqrt{5}}{2}+ \dfrac{3}{2})\\
&=& -4A_3(\phi/2)^{n+1} - 4A_4(\psi/2)^{n+1}  - \dfrac{n+2}{2^{n}} + 4
\eeqn


よって, 求める確率は
\beqn
4-\dfrac{10+4\sqrt{5}}{5}\parena{\dfrac{1+\sqrt{5}}{4}}^{n+1} - \dfrac{10-4\sqrt{5}}{5}\parena{\dfrac{1-\sqrt{5}}{4}}^{n+1} - \dfrac{n+2}{2^{n}}
\eeqn
\end{comment}
